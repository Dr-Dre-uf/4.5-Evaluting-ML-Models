...
if page == "Data Explorer":
    ...
    with st.expander("Filter Options", expanded=True):
        c1, c2 = st.columns(2)
        with c1:
            age_range = st.slider("Select Age Range", 21, 81, (21, 81))
        with c2:
            bmi_range = st.slider("Select BMI Range", 18.0, 67.0, (18.0, 67.0))
...
# === PAGE: MODEL TRAINING ===
    ...
    if track == "Clinical Science":
        ...
        2. **Algorithm & Parameters**: Select an algorithm and adjust its strictness. In a clinical context, adjusting regularization (C) changes how heavily the model penalizes complex risk factors.
        ...
        2. **Hyperparameter Tuning**: Select an algorithm and tune its parameters. For Logistic Regression, 'C' controls regularization strength. For SVM, you can also alter the kernel type.
    ...
    with col_params:
        if model_type == "Logistic Regression":
            c_param = st.slider("Regularization Strength (C):", min_value=0.01, max_value=10.0, value=1.0, step=0.1)
            max_iter_param = st.slider("Maximum Iterations:", min_value=100, max_value=2000, value=1000, step=100)
        else:
            c_param_svm = st.slider("Regularization Strength (C):", min_value=0.1, max_value=10.0, value=1.0, step=0.1)
            kernel_param = st.selectbox("Kernel Type:", ["linear", "poly", "rbf", "sigmoid"], index=2)
            gamma_param = st.selectbox("Gamma:", ["scale", "auto"], index=1)
    ...
    if st.button("Train and Evaluate Model"):
        if not selected_features:
            ...
            # Using same test_size and random_state as notebook
